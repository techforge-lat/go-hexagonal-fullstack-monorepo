...
func (r Repository) WithTx(tx ports.Transaction) ports.UsersOriginEnumRepository {
	return &Repository{
		db: r.db,
		tx: tx.GetTx(),
	}
...
func (r Repository) CreateBulk(ctx context.Context, entities types.List[entity.UsersOriginEnumCreateRequest]) error {
	query := insertQuery
	for _, entity := range entities {
		query = query.WithValues(entity.Code, entity.Name)
	}
...
func (r Repository) Create(ctx context.Context, entity entity.UsersOriginEnumCreateRequest) error {
	result, err := insertQuery.WithValues(entity.Code, entity.Name).ToSQL()
	if err != nil {
		return fault.Wrap(err)
	}
...
func (r Repository) Update(ctx context.Context, entity entity.UsersOriginEnumUpdateRequest, filters ...dafi.Filter) error {
	result, err := updateQuery.WithValues(entity.Code, entity.Name).Where(filters...).ToSQL()
	if err != nil {
		return fault.Wrap(err)
	}
...
func (r Repository) Find(ctx context.Context, criteria dafi.Criteria) (entity.UsersOriginEnum, error) {
	if err := dafi.ValidateSelectFields(criteria.SelectColumns, sqlColumnByDomainField); err != nil {
		return entity.UsersOriginEnum{}, fault.Wrap(err).Code(fault.BadRequest)
	}
...
func (r Repository) List(ctx context.Context, criteria dafi.Criteria) (types.List[entity.UsersOriginEnum], error) {
	if err := dafi.ValidateSelectFields(criteria.SelectColumns, sqlColumnByDomainField); err != nil {
	...
	var list types.List[entity.UsersOriginEnum]
