-- Migration: rename_customers_schema_to_relationships
-- Description: Rename customers schema to relationships and update all foreign key constraints

BEGIN;

-- 1. Drop all foreign key constraints that reference customers schema tables
...
ALTER TABLE customers.contacts DROP CONSTRAINT IF EXISTS contacts_company_id_fkey;
ALTER TABLE customers.renewal_alerts DROP CONSTRAINT IF EXISTS renewal_alerts_company_id_fkey;

-- 2. Rename the schema
ALTER SCHEMA customers RENAME TO relationships;

-- 3. Recreate all foreign key constraints with the new schema name
-- Auth Schema constraints
ALTER TABLE auth.organization_customers ADD CONSTRAINT organization_customers_customer_id_fkey 
    FOREIGN KEY (customer_id) REFERENCES relationships.companies(id) ON DELETE CASCADE;
...
ALTER TABLE relationships.contacts ADD CONSTRAINT contacts_company_id_fkey 
    FOREIGN KEY (company_id) REFERENCES relationships.companies(id) ON DELETE CASCADE;
