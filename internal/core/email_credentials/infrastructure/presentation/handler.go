...
type Handler struct {
	useCase ports.EmailCredentialsUseCase
}

func NewHandler(useCase ports.EmailCredentialsUseCase) *Handler {
	return &Handler{useCase: useCase}
}

func (h Handler) Create(c echo.Context) error {
	req := entity.EmailCredentialsCreateRequest{}
...
func (h Handler) Update(c echo.Context) error {
	req := entity.EmailCredentialsUpdateRequest{}
	if err := c.Bind(&req); err != nil {
		return fault.Wrap(err).Code(fault.UnprocessableEntity)
	}
